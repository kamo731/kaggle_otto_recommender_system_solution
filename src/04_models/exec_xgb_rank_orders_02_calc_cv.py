...
from exec_xgb_rank_orders_01_train import common_settings, get_session_lengths
...
def main():
    ...
    for fold, (_, valid_idx) in enumerate(
        ...
        for i in range(n_split):
            ...
            x_val_part = x_val_part.sort_values("session")
            session_length = get_session_lengths(x_val_part)
            df_pred_part = x_val_part[["session", "aid"]].copy().to_pandas()
            x_val_part = x_val_part.drop(["session", "aid"], axis=1).to_pandas()

            dvalid = xgb.DMatrix(data=x_val_part, group=session_length)
