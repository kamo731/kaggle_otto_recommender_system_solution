...
from exec_xgb_rank_orders_01_train import common_settings
from loguru import logger

from globals import FEATURES, ID2TYPE, LOGS, OUTPUT, SUBMISSION
from utils import concat_session_and_additional_candidates, make_sub, merge_features_valid_only, timer

np.random.seed(42)
feature_info_test = {
    # TODO: concatのmergeを先にしないとindexがおかしくなる
    FEATURES / "base_session_and_aid_features_test_type_2.parquet": ["session", "aid"],
    FEATURES / "word2vec_features_session_and_aid_last1_test_type_2.parquet": ["session", "aid"],
    FEATURES / "word2vec_features_session_and_aid_last5_test_type_2.parquet": ["session", "aid"],
    FEATURES / "word2vec_features_session_and_aid_all_test_type_2.parquet": ["session", "aid"],
    FEATURES / "cvm_features_all_test_type_2.parquet": ["session", "aid"],
    FEATURES / "cvm_features_last1_test_type_2.parquet": ["session", "aid"],
    FEATURES / "cvm_features_last5_test_type_2.parquet": ["session", "aid"],
    # base
...
def main():
    ...
    with timer("predict"):
        df_test = cd.read_parquet(
            FEATURES / "base_preprocess_candidates_test_type_2.parquet",
            columns=["session", "aid"],
    ...
    with timer("create submission", logger):
        ...
        # recall_for_filename = str(recall).replace(".", "_")[:7]
        submission.to_csv(
            # sub_dir / f"{current_time}_ver_{version}_recall{recall_for_filename}_submission.csv", index=False
