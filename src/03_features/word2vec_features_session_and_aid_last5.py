...
def main():
    ...
    # merge
    svd_cols_last5 = [f"svd_{x}_last5" for x in range(10)]
    use_cols = ["session"] + svd_cols_last5
    candidates = candidates.merge(df_session[use_cols], how="left", on="session")

    # calc diff
    diff_cols_candidates_last5 = [f"svd_{x}_diff_candidates_last5" for x in range(10)]
    candidates[diff_cols_candidates_last5] = candidates[svd_cols_candidates].values - candidates[svd_cols_last5].values
    candidates[f"svd_total_diff_candidates_last5"] = candidates[diff_cols_candidates_last5].abs().sum(axis=1)

    # drop cols
    candidates = candidates.drop(svd_cols_candidates + svd_cols_last5, axis=1)

    candidates = reduce_mem_usage(candidates)

    assert candidates.isna().sum().sum() == 0, "nullの特徴量が存在します。"
    candidates.to_parquet(FEATURES / f"word2vec_features_session_and_aid_last5_{phase}_type_{type_id}.parquet")
